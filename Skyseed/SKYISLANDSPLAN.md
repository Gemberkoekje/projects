...
**Seeds built — all 10:** Forest (`skyseed:forest`), Rocky (`skyseed:rocky`), Desert (`skyseed:desert`), Mushroom (`skyseed:mushroom`), Frozen (`skyseed:frozen`), Meadow (`skyseed:meadow`), Badlands (`skyseed:badlands`, `fill_bands` strata), Ancient (`skyseed:ancient`) + Lush (`skyseed:lush`) (`underside` hanging decoration), and **Aquatic** (`skyseed:aquatic`, `pond.plants` water plants) — plus a **Large variant of every type** (v0.3.0): `forest_large`, `rocky_large` (a tall emerald-bearing mountain), `desert_large` (with an oasis), `mushroom_large`, `frozen_large` (a frozen lake), `meadow_large`, `badlands_large` (a towering mesa), `ancient_large`, `lush_large`, and `aquatic_large` (a proper deep lake). The full island set is complete; remaining work is polish (see the per-island "As built" notes). The sibling plans are now largely built too — `THROWMODEPLAN.md` (dual throw modes) and `SKYVILLAGESPLAN.md` (Hamlet → Trade Post → Village Center) are done, `SKYANIMALSPLAN.md` (the five Animal Islands + sprinkles) is done, and `SKYSTRUCTURESPLAN.md` is in progress (Dungeon, Ruined Portal, Desert/Jungle Temple, Witch Hut, Pillager Outpost and Trail Ruins built — plus rare-structure variants of several seeded onto big islands; Mansion Room / Trial Chamber remain). See each plan's status section.

**Engine features built:** datapack theme codec; biome-aware generation; Y-band overrides (`min_y`/`max_y`); contained ponds; off-rim waterfall cascades; hand-built mangroves (`skyseed:mangrove`); per-column `surface_scatter`; snow peaks; curated start island; void world preset with multi-noise biomes; structures disabled; tick-budgeted placement; overlap safety; charge-to-throw distance; Patchouli guide (per-island entries, advancement-gated).
...
When this note was first written, several planned types existed only as **Forest biome-overrides**. **Mushroom and Aquatic have since been promoted to their own dedicated seeds** (listed above). Forest's biome overrides still deliver flavors that have no seed of their own — the snowy/taiga look (`#is_taiga` → podzol + spruce, `grove`/snowy biomes → spruce + snow) and the small lake/river water islands (`#is_ocean`/`#is_river` → pond + waterfalls, `mangrove_swamp` → mud + mangroves) — which could become dedicated seeds in future.
...
> **As built:** the Y-banded ore tables and the snow peak (Y ≥ 130 or snowy biome) are live. The bands were **tuned toward the vanilla ore-by-depth curve in v0.17** — deep throws give deepslate diamond/redstone/gold/lapis (the richest haul), mid throws an iron-and-**copper** peak with coal, and high/peak throws coal and iron but none of the deep treasures (so a high island gives less than a low one). **Copper is a common ore now**, and iron/coal/copper grow in veins of 4–8 at normal heights. The **Mountain variant** (emerald ore + a tall, peaked silhouette + snow cap + goats) ships as the separate **Large Rocky** seed (`skyseed:rocky_large`, v0.3.0) rather than a base-Rocky variant. **Not yet built:** the granite/diorite/andesite surface variants, rare lava pools, and gravel surface patches on base Rocky (base Rocky still has `variants: []`; Large Rocky adds mountain / snowcapped / scree).
...
> **As built (`skyseed:lush`):** moss surface / rooted-dirt fill / stone core; hand-built azalea trees (`skyseed:azalea` — vanilla's `rooted_azalea_tree` feature refuses to place on floating islands); big + small dripleaf, moss carpet, torchflower, and pitcher plant on top; hanging roots, berry-laden cave vines, and spore blossoms on the underside. Garden, glow (berry-heavy underside), and ancient-garden variants. Recipe moss block + azalea. A central pool holds **axolotls** (v0.2.2). A bigger **Large Lush** seed (`skyseed:lush_large`) grows a deep pool that also houses **glow squid** (they beach in a small pond). **Not done:** side vines (need a face).
