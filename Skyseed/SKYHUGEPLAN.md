...
| Huge seed | Shape | Why |
|---|---|---|
| `huge_aquatic` | **single (big ocean)** | a single big island that's *mostly* a central lake/ocean (`Pond.extent` **0.80** fills it, thin shore). The cluster route read as boring separate islands — and ponds are centre-only + half-radius-capped, so a cluster can't hold a real lake. Cluster stays viable for *non-water* seeds (mushroom). *(v0.123.0: the biome-form ponds had a radius below the extent cap → a tiny pond on a vast sand bank; fixed so radius > cap and extent does the limiting. v0.125.0: + a `slope`d basin floor — full depth in a flat centre, shallowing to the shore — so the edge rests on the rim's body and eases in, instead of a sheer deep edge that spilled through the bottom.)* |
...
3. **Rare structures — DONE (v0.126.0).** Every structural huge biome carries a rare centre build (`rare_structures`,
   replacing the normal jigsaw + wildlife on a roll): **Ancient → the Ancient City** (NEW: deepslate plaza, dark-oak
   reliquary over soul fire, can-summon sculk shriekers = Warden danger, 3× `ancient_city` loot; 6%); and existing
   structures reused — **Forest → Woodland Mansion** (5%), **Desert → Desert Pyramid** (6%), **Frozen → Igloo** (6%),
   **Meadow → Pillager Outpost** (6%), **Rocky → spawner Dungeon** (6%), **Badlands → Trial Chamber** (5%). Aquatic's
   grand Ocean Monument shipped earlier (0.119.0); Mushroom + Lush keep their caves as the draw. Chances are first-pass.
4. **Roll out — DONE (v0.124.0).** The full huge roster ships: **forest, rocky, desert, mushroom, frozen, meadow,
   badlands, ancient, lush, aquatic** — all real, gated (`ender pearl / *_large seed / blaze powder`) seeds with the
   biome adaptation their base theme has, caves (land themes), unique icons, guides, advancements, and debug seeds.
   Generated by scaling the base themes (per-theme dome/radius/cave tuning: a towering Rocky mountain, a Badlands mesa,
   a deepslate Ancient cave-host, …). Sizes/cave-density/recipe-bulk are first-pass — tune in playtest. **Phases 1–4
   are all DONE; the huge tier is feature-complete.** Earlier framing: (the bigger Ocean Monument is in; Ancient City / large Mansion /
   dungeon menu in §3).
