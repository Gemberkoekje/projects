...
- **A "Huge" island tier** (`SKYHUGEPLAN.md`) — overworld islands well above `*_large`: a bigger single landmass (radius bump + `max_under_depth` plateau cap) or a `cluster_offsets` archipelago, chosen per seed; with **internal cave systems** (carved like a pond, decorated from the underside palette — dripstone, cave vines, glow lichen) and a **rare** large structure inside (a jigsaw dungeon, the large woodland mansion — via the existing `RareStructure` roll). Gated by a recipe whose middle row is `ender pearl / *_large seed / blaze powder` (the large seed between an End and a Nether farm drop — huge demands farming both late chapters), wrapped in the theme's bulk block.
