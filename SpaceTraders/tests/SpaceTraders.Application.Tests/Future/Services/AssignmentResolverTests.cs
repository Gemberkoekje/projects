...
public sealed class AssignmentResolverTests
{
    ...
    [Fact]
    public async Task ResolveAsync_ConstructionGoal_ReturnsSupplyConstructionGoal()
    {
        var waypoints = Substitute.For<IWaypointRepository>();
        var markets = Substitute.For<IMarketRepository>();
        waypoints.GetBySystemAsync("X1-AB", Arg.Any<CancellationToken>())
            .Returns([MakeWaypoint("X1-AB-STATION", "X1-AB", "ORBITAL_STATION", 0, 0)]);

        var ship = MakeShip();
        var goal = new ResourceProductionGoal(
            "ADVANCED_CIRCUITRY",
            50,
            ResourceProductionPurpose.Construction,
            FleetGoalPriority.Construction,
            DeliveryWaypoint: "X1-AB-GATE",
            ProtectTradeSymbolFromSale: true);

        var resolver = Build(waypoints, markets);
        var result = await resolver.ResolveAsync(ship, goal);

        result.Should().BeOfType<SupplyConstructionGoal>();
        ((SupplyConstructionGoal)result!).ConstructionSiteWaypointSymbol.Should().Be("X1-AB-GATE");
    }

    [Fact]
    public async Task FindBestSellMarketAsync_PrefersPreferredWaypoint_WhenItBuysTradeSymbol()
    {
        var waypoints = Substitute.For<IWaypointRepository>();
        var markets = Substitute.For<IMarketRepository>();
        ...
        var station = MakeWaypoint("X1-AB-STATION", "X1-AB", "ORBITAL_STATION", 0, 0);
        var marketNear = MakeMarket("X1-AB-MARKET-NEAR", "X1-AB", 15, 0);
        var marketPreferred = MakeMarket("X1-AB-MARKET-PREFERRED", "X1-AB", 60, 0);

        waypoints.GetBySystemAsync("X1-AB", Arg.Any<CancellationToken>())
            .Returns([station, marketNear, marketPreferred]);

        markets.GetAllSnapshotsAsync(Arg.Any<CancellationToken>())
            .Returns([
                MakeMarketSnapshot("X1-AB-MARKET-NEAR", "X1-AB", imports: ["IRON_ORE"]),
                MakeMarketSnapshot("X1-AB-MARKET-PREFERRED", "X1-AB", imports: ["IRON_ORE"]),
            ]);

        var resolver = Build(waypoints, markets);
        var result = await resolver.FindBestSellMarketAsync(MakeShip(), "IRON_ORE", "X1-AB-MARKET-PREFERRED");

        result.Should().Be("X1-AB-MARKET-PREFERRED");
    }
}
