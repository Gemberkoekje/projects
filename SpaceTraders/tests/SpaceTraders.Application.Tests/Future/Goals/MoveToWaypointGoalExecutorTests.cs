...
public sealed class MoveToWaypointGoalExecutorTests
{
    ...
    [Fact]
    public async Task ExecuteStepAsync_DockedAtTarget_CompletesWithoutDockCommand()
    {
        var result = await CreateExecutor().ExecuteStepAsync(Ship("X1-AB-WP2", "DOCKED"), Goal("X1-AB-WP2"), Ctx(), CancellationToken.None);

        result.Outcome.Should().Be(GoalExecutionOutcome.Completed);
        await _inOrbit.DidNotReceive().DockAsync(Arg.Any<string>(), Arg.Any<CancellationToken>());
    }
    ...
    [Fact]
    public async Task ExecuteStepAsync_Docked_NotAtTarget_RefuelsWhenFuelAvailableAtWaypoint()
    {
        var ctx = Ctx() with { CurrentWaypointSellsFuel = true };
        var result = await CreateExecutor().ExecuteStepAsync(
            Ship("X1-AB-WP1", "DOCKED", fuel: 20, fuelCap: 100),
            Goal("X1-AB-WP2"),
            ctx,
            CancellationToken.None);

        result.Outcome.Should().Be(GoalExecutionOutcome.WaitingForArrival);
        await _docked.Received(1).RefuelAsync("SHIP-1", false, Arg.Any<CancellationToken>());
        await _docked.Received(1).OrbitAsync("SHIP-1", Arg.Any<CancellationToken>());
    }

    [Fact]
    public async Task ExecuteStepAsync_Docked_NotAtTarget_DoesNotRefuelWhenFuelUnavailable()
    {
        var ctx = Ctx() with { CurrentWaypointSellsFuel = false };
        var result = await CreateExecutor().ExecuteStepAsync(
            Ship("X1-AB-WP1", "DOCKED", fuel: 20, fuelCap: 100),
            Goal("X1-AB-WP2"),
            ctx,
        ...
        result.Outcome.Should().Be(GoalExecutionOutcome.WaitingForArrival);
        await _docked.DidNotReceive().RefuelAsync(Arg.Any<string>(), Arg.Any<bool>(), Arg.Any<CancellationToken>());
        await _docked.Received(1).OrbitAsync("SHIP-1", Arg.Any<CancellationToken>());
    }
}
