...
public sealed class ScoutWaypointGoalExecutorTests
{
    ...
    [Fact]
    public async Task ExecuteStepAsync_InOrbit_AtTarget_DocksRefreshesAndCompletes()
    {
    ...
    {
        var ctx = new ShipGoalContext { CurrentWaypointSellsFuel = true };
    ...
    {
        var ctx = new ShipGoalContext { CurrentWaypointSellsFuel = false };
        var result = await CreateExecutor().ExecuteStepAsync(
            Ship("X1-AB-WP1", "DOCKED"),
            Goal("X1-AB-TARGET"),
            ctx,
