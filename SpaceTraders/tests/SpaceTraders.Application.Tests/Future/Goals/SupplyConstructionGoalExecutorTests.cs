...
public sealed class SupplyConstructionGoalExecutorTests
{
    ...
    private static ShipModel Ship(
        ...
        new("SHIP-1", "X1-AB", waypoint, status, "CRUISE", 80, fuelCap,
            CargoCurrent: inventory?.Sum(i => i.Units) ?? 0,
            CargoCapacity: 40,
    ...
    [Fact]
    public async Task ExecuteStepAsync_Docked_NotAtSite_FabMatsAffordable_BuysInBulk()
    {
        var market = new MarketSnapshot(
            "X1-AB-WP1",
            "X1-AB",
            [new TradeGoodSnapshot("FAB_MATS", "EXPORT", 2_400, 2_600, 60, "MODERATE")],
            [],
            ["FAB_MATS"],
            []);
        var ctx = new ShipGoalContext
        {
            CurrentMarketSnapshot = market,
            SpendableCredits = 200_000,
            FabMatsBuyThreshold = 2_500,
            FabMatsTransactionSize = 60,
        };

        var result = await CreateExecutor().ExecuteStepAsync(
            Ship("X1-AB-WP1", "DOCKED"),
            new SupplyConstructionGoal { ConstructionSiteWaypointSymbol = "X1-AB-SITE", TradeSymbol = "FAB_MATS" },
            ctx,
            CancellationToken.None);

        result.Outcome.Should().Be(GoalExecutionOutcome.Progressing);
        await _docked.Received(1).BuyCargoAsync("SHIP-1", "FAB_MATS", 40, Arg.Any<CancellationToken>());
    }

    [Fact]
    public async Task ExecuteStepAsync_Docked_NotAtSite_FabMatsAboveThreshold_DoesNotBuy()
    {
        var market = new MarketSnapshot(
            "X1-AB-WP1",
            "X1-AB",
            [new TradeGoodSnapshot("FAB_MATS", "EXPORT", 2_700, 2_900, 60, "MODERATE")],
            [],
            ["FAB_MATS"],
            []);
        var ctx = new ShipGoalContext
        ...
        result.Outcome.Should().Be(GoalExecutionOutcome.WaitingForArrival);
        await _docked.DidNotReceive().BuyCargoAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<int>(), Arg.Any<CancellationToken>());
        await _docked.Received(1).OrbitAsync("SHIP-1", Arg.Any<CancellationToken>());
    }
}
