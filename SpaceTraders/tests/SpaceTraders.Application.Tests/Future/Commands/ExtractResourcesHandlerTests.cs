...
public sealed class ExtractResourcesHandlerTests
{
    ...
    [Fact]
    public async Task ExtractResources_WhenExtractionYieldsCargo_PublishesDeliverableObtainedEvent()
    {
        var port = Substitute.For<ISpaceTradersPort>();
        var ships = Substitute.For<IShipRepository>();
        var goals = Substitute.For<IShipGoalRepository>();
        ...
        await bus.Received(1).PublishAsync(
            Arg.Is<DeliverableObtainedEvent>(e =>
                e.ShipSymbol == "SHIP-1"
                && e.TradeSymbol == "IRON_ORE"
                && e.UnitsObtained == 10),
            Arg.Any<DeliveryOptions>());
    }

    [Fact]
    public async Task ExtractResources_WhenExtractionHasNoYield_DoesNotPublishDeliverableObtainedEvent()
    {
        var port = Substitute.For<ISpaceTradersPort>();
        var ships = Substitute.For<IShipRepository>();
        var goals = Substitute.For<IShipGoalRepository>();
        ...
        surveys.GetBestActiveSurveyAsync("X1-AB-001", Arg.Any<string>(), Arg.Any<CancellationToken>())
            .Returns(new SurveyModel(string.Empty, "X1-AB-001", [], DateTimeOffset.MinValue, string.Empty));
        port.ExtractResourcesAsync("SHIP-1", Arg.Any<CancellationToken>())
            .Returns(new ExtractionActionResult(string.Empty, 0, new CargoModel(10, 60), 60));

        var handler = new ExtractResourcesHandler(port, ships, goals, scheduler, waypoints, surveys, assignments, bus, NullLogger<ExtractResourcesHandler>.Instance);
        await handler.Handle(new ExtractResourcesCommand("SHIP-1"), CancellationToken.None);

        await bus.DidNotReceive().PublishAsync(Arg.Any<DeliverableObtainedEvent>(), Arg.Any<DeliveryOptions>());
    }
}
