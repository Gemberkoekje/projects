    ...
    ILogger<AssignShipToGoalCommandHandler> logger)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var protectTradeSymbolFromSale = command.Goal.Kind == FleetGoalKind.Construction
            && (command.Goal.TradeSymbol?.Equals("ADVANCED_CIRCUITRY", StringComparison.OrdinalIgnoreCase) == true
                || command.Goal.TradeSymbol?.Equals("ADVANCED_CIRCUIT", StringComparison.OrdinalIgnoreCase) == true);

        var preferredSellWaypoint = command.Goal.IsSupplyChainPrecursor
            ? command.Goal.OriginWaypoint
            : null;

        var deliveryWaypoint = command.Goal.Kind == FleetGoalKind.Construction
            ? command.Goal.DestinationWaypoint
            : null;
        ...
        var productionGoal = new ResourceProductionGoal(
            ...
            PreferredSellWaypoint: preferredSellWaypoint,
            ProtectTradeSymbolFromSale: protectTradeSymbolFromSale);
