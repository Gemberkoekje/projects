...
/// <summary>
/// Phase 1 renderer: fullbright spectral path tracing of the real maze scene on
/// the GPU. Builds a single BLAS/TLAS from the packed scene, uploads the spectral
/// tables and per-primitive data, and dispatches <c>PathTrace.hlsl</c> once per
/// frame into a persistent per-pixel accumulation buffer, then presents the
/// resolved sRGB image (windowed) or reads it back for validation (headless).
///
/// All buffers are bound as root descriptors (structured buffers are legal root
/// SRV/UAVs); only the output texture needs a descriptor heap. This mirrors the
/// proven Phase 0 <see cref="GpuRayTracer"/> resource patterns.
/// </summary>
internal sealed class Phase1Renderer : IDisposable
{
    ...
    // Scene / spectral / accumulation resources.
    ...
    public Phase1Renderer(
        int width, int height,
        PackedScene scene, SpectralResources spectral, Camera camera,
    ...
    // ── Constant buffer layout (matches PathTrace.hlsl's cbuffer) ──────

    [StructLayout(LayoutKind.Sequential)]
    private struct Phase1Constants
    {
        public float CamPosX, CamPosY, CamPosZ, Pad0;
        public float RotX, RotY, RotZ, RotW;
        public float TanHalfFov, AspectTanHalfFov, InvWidth, InvHeight;
        public float ImgPlaneZ, DeterministicCorrection, Pad1, Pad2;
        public uint Width, Height, MaxSampleCount, ResetFlag;
        public uint NumPrimitives, SubPixelJitter, Pad3, Pad4;
    }
    ...
    private void CreateAccumulationResources()
    {
        ...
        // float4 per pixel for the running-mean accumulator.
    ...
    private void CreatePipeline()
    {
        ...
        byte[] bytecode = ShaderCompiler.CompileCompute(hlsl, "CSMain", "PathTrace.hlsl");
    ...
    {
        string path = Path.Combine(AppContext.BaseDirectory, "Shaders", "PathTrace.hlsl");
        if (!File.Exists(path))
    ...
    private void UpdateConstants(bool reset)
    {
        ...
        var constants = new Phase1Constants
        {
        ...
        Span<Phase1Constants> dest = _constantBuffer.Map<Phase1Constants>(0, 1);
        dest[0] = constants;
