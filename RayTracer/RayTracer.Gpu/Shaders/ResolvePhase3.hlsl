// Phase 3 — resolve pass of the temporal pipeline.
//
// One thread per pixel. Reads the trace pass's accumulation buffer + G-buffer
// (world hit point, oriented normal, hit mask) and the previous frame's TAA
// history, then:
//   * spatially filters the accumulated mean (bilateral or box, motion-gated),
//   * reprojects the current world hit point into the previous frame using the
//     previous camera, bilinearly fetches history with per-tap validity weights,
//   * rejects on low validity, reprojection error, or normal disagreement,
//   * neighborhood-clamps accepted history and temporally blends it with the
//     current color,
//   * writes the resolved sRGB image and the next-frame history.
//
// Line-for-line port of RayTracer.Core's Phase3Reference.cs, which the unit
// tests pin to the CPU renderer's JobSystem.TaaResolver. Requires SM 6.0+.
