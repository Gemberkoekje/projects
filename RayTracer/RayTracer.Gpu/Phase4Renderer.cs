...
/// <summary>
/// Phase 4 renderer: the Phase 3 temporal pipeline with <b>volumetrics</b>. The
/// trace pass (<c>PathTracePhase4.hlsl</c>) is the Phase 3 spectral path tracer
/// plus a camera-segment single-scattering march (smoke/fog) composited onto the
/// surface XYZ before accumulation; the resolve pass is unchanged, so Phase 3's
/// <c>ResolvePhase3.hlsl</c> (bilateral filter + TAA reprojection) is reused
/// verbatim.
///
/// The volumetric march tints its in-scatter by per-light colour, so this host
/// additionally uploads a light-colour buffer (t5) alongside the Phase 2/3
/// position buffer (t4). The march parameters come from a <see cref="VolumetricOptions"/>
/// packed into the trace constant buffer; the marcher is a line-for-line port of
/// <see cref="Phase4Reference"/>, pinned to <c>JobSystem.IntegrateVolumetricSegment</c>
/// by the unit tests.
///
/// As with Phase 1/2/3 the D3D12 scaffolding is intentionally duplicated rather
/// than refactoring the earlier (separately dev-box-validated) hosts.
/// </summary>
internal sealed class Phase4Renderer : IDisposable
{
    ...
    public Phase4Renderer(
        ...
        uint filterRadius = 1, bool edgeAwareFilter = false, bool biomeIndicator = false)
    {
        _width = width;
        _height = height;
        _scene = scene;
    ...
    {
        // Trace pass: SRVs t0-t5, UAVs u0-u2/u4-u8, CBV b0. No Output (u3).
        var traceParams = new[]
        {
        ...
        byte[] traceBytecode = ShaderCompiler.CompileCompute(
            LoadShaderSource("PathTracePhase4.hlsl"), "CSMain", "PathTracePhase4.hlsl");
        ...
        // Phase 4 adds FogIn (u9) and composites the fog; otherwise identical to
        // the Phase 3 resolve.
        ...
        byte[] resolveBytecode = ShaderCompiler.CompileCompute(
            LoadShaderSource("ResolvePhase4.hlsl"), "CSMain", "ResolvePhase4.hlsl");
    ...
    // ── Acceleration structures (identical to Phase 1/2/3) ────────────
    ...
    /// <summary>Sets the fog animation clock (seconds). The volumetric turbulence
    /// domain drifts with this, so advancing it each frame makes the fog roll;
    /// leaving it at 0 keeps the fog static (as the self-test/capture do).</summary>
    ...
    private void RecordTraceAndResolve()
    {
        ...
        // Make the trace pass's accumulation + G-buffer + fog writes visible to
        // the resolve pass (they alias the same UAVs across two dispatches).
    ...
    /// path (soft reset + spatial filter + tighter reprojection + halved march steps).</summary>
